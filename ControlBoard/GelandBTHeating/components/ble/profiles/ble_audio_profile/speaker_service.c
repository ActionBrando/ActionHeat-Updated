...
extern os_timer_t timer_refresh; // ����ˢ�¼Ĵ��������Լ���ʾ��
...
#define SBC_DATA_FLASH_ADDR (USER_FLASH_BASE_ADDR + FLASH_PAGE_SIZE) // ��Ƶ������ʼ��ַ
/*********************************************************************
...
void store_sbc_to_flash(uint8_t *p_buf, uint32_t store_len)
{
    ...
    while (sample_sz > 0)
    {
        if (sbc_sotre_env.last_offset < FLASH_PAGE_SIZE && sbc_sotre_env.last_offset > 0)
        {
            ...
            {
                SPEAKER_LOG("x,");
                flash_write(sbc_sotre_env.start_base + sbc_sotre_env.last_page_idx * FLASH_PAGE_SIZE + sbc_sotre_env.last_offset, FLASH_PAGE_SIZE - sbc_sotre_env.last_offset, p_buf + pos);
            ...
            {
                SPEAKER_LOG("y,");
                flash_write(sbc_sotre_env.start_base + sbc_sotre_env.last_page_idx * FLASH_PAGE_SIZE + sbc_sotre_env.last_offset, sample_sz, p_buf + pos);
                pos += sample_sz;
        ...
        else
        {
            if (sample_sz >= FLASH_PAGE_SIZE)
            {
                ...
                flash_write(sbc_sotre_env.start_base + sbc_sotre_env.last_page_idx * FLASH_PAGE_SIZE + sbc_sotre_env.last_offset, FLASH_PAGE_SIZE, p_buf + pos);
...
static void sp_gatt_write_cb(uint8_t *write_buf, uint16_t len, uint16_t att_idx, uint8_t conn_idx)
{
    ...
    case SPEAKER_IDX_AUDIO_RX_VALUE:
    {
        ...
        SPEAKER_LOG("%d,", len);
        store_sbc_to_flash(write_buf, len); // �����յ�����д��flash
    }
    break;
    }
}
