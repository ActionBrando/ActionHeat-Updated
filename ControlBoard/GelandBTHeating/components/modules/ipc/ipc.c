...
static void program_relay(void)
{
    ...
    while (1)
    {
        ...
        if ((opcode != UP_OPCODE_READ) && (opcode != UP_OPCODE_READ_RAM))
        {
            while (length--)
            {
        ...
        if ((opcode == UP_OPCODE_READ) || (opcode == UP_OPCODE_READ_RAM) || (opcode == UP_OPCODE_CALC_MD5) || (opcode == UP_OPCODE_CALC_CRC32))
        {
            while (length--)
            {
